...
/*   Updated: 2019/11/04 10:30:22 by rjaakonm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// FT_GET_NEXT_LINE
	...
	printf("\nft_get_next_line:\n");
	printf("\033[0m");
	printf("if no arguments, read input, if 1 argument, print all lines, \
		if 2 arguments, print first 2 lines from each\n");
	int			fd;
	int			fd1;
	int			fd2;
	char		*line;
...
/*		fd = open(av[1], O_RDONLY);
		...
		ft_get_next_line(fd1, &line);
		printf("returned line from file 1: %s\n", line);
		ft_get_next_line(fd2, &line);
